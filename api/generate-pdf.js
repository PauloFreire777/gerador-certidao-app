// api/generate-pdf.js - VERSÃO FINAL 100% SINCRONIZADA COM O PREVIEW
...
// FUNÇÃO DE HERDEIROS TOTALMENTE CORRIGIDA
function generateHeirsHtml(heirs, allAdvogados, level = 0) {
    ...
    return heirs.map(h => {
        ...
        <div class="preview-card no-break" style="margin-left: ${level * 20}px;">
            ...
            ${h.idProcuracao ? `<div class="info-procuracao"><p><strong>Procuração (ID):</strong> <span>${h.idProcuracao}</span></p></div></div>` : ''}
            ${advogado ? `<div class="info-advogado"><p><strong>Advogado(a):</strong> <span>${advogado.nome} (OAB: ${advogado.oab})</span></p></div></div>` : ''}
            ...
            ${h.estado === 'Incapaz' ? `<div class="preview-sub-card warning no-break">
                ...
                ${h.curador.idProcuracao ? `<div class="info-procuracao" style="margin-top: 8px;"><p><strong>Procuração Curador (ID):</strong> <span>${h.curador.idProcuracao}</span></p></div></div>` : ''}
                ${curadorAdvogado ? `<div class="info-advogado" style="margin-top: 8px;"><p><strong>Advogado(a) do Curador:</strong> <span>${curadorAdvogado.nome} (OAB: ${curadorAdvogado.oab})</span></p></div></div>` : ''}
            </div>` : ''}
            ...
            ${(h.estadoCivil === 'Casado(a)' || h.estadoCivil === 'União Estável') ? `<div class="preview-sub-card no-break">
                ...
                ${h.conjuge.idProcuracao ? `<div class="info-procuracao" style="margin-top: 8px;"><p><strong>Procuração Cônjuge (ID):</strong> <span>${h.conjuge.idProcuracao}</span></p></div></div>` : ''}
                ${conjugeAdvogado ? `<div class="info-advogado" style="margin-top: 8px;"><p><strong>Advogado(a) do Cônjuge:</strong> <span>${conjugeAdvogado.nome} (OAB: ${conjugeAdvogado.oab})</span></p></div></div>` : ''}
            </div>` : ''}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (data.falecidos.length > 0) {
            sectionCounter++;
            htmlSections += `<div class="preview-section"><h3>${sectionCounter}. De Cujus</h3>${data.falecidos.map(f => `<div class="preview-card"><p><strong>Nome:</strong><span>${f.nome}</span></p><p><strong>Data do Óbito:</strong><span>${formatDate(f.dataFalecimento)}</span></p><p><strong>Docs. Pessoais (ID):</strong><span>${f.documentos || 'Não informado'}</span></p><p><strong>Cert. Óbito (ID):</strong><span>${f.idCertidaoObito || 'Não informado'}</span></p></div></div>`).join('')}</div>`;
        }
        if (data.inventariante.nome || herdeirosValidos.length > 0) {
            sectionCounter++;
            htmlSections += `<div class="preview-section"><h3>${sectionCounter}. Inventariante, Herdeiros e Sucessores</h3>${data.inventariante.nome ? `<div class="preview-card no-break"><h4>Inventariante</h4><p><strong>Nome:</strong><span>${data.inventariante.nome}</span></p><p><strong>Parentesco:</strong><span>${data.inventariante.parentesco || 'Não informado'}</span></p><p><strong>Docs. Pessoais (ID):</strong><span>${data.inventariante.documentos || 'Não informado'}</span></p><p><strong>Termo de Comp. (ID):</strong><span>${data.inventariante.idTermoCompromisso || 'Não informado'}</span></p>${data.inventariante.idProcuracao ? `<div class="info-procuracao"><p><strong>Procuração (ID):</strong><span>${data.inventariante.idProcuracao}</span></p></div></div>` : ''}${getAdvogadoById(data.inventariante.advogadoId, data.processo.advogados) ? `<div class="info-advogado"><p><strong>Advogado(a):</strong><span>${getAdvogadoById(data.inventariante.advogadoId, data.processo.advogados).nome} (OAB: ${getAdvogadoById(data.inventariante.advogadoId, data.processo.advogados).oab})</span></p></div></div>` : ''}</div>` : ''}${generateHeirsHtml(herdeirosValidos, data.processo.advogados)}</div>`;
        }
        if (data.renuncia.houveRenuncia && data.renuncia.renunciantes.length > 0) {
            sectionCounter++;
            htmlSections += `<div class="preview-section"><h3>${sectionCounter}. Renúncia de Direitos</h3>${data.renuncia.renunciantes.map(r => `<div class="preview-card"><p><strong>Renunciante:</strong><span>${getHeirNameById(r.herdeiroId, herdeirosValidos)}</span></p><p><strong>Tipo:</strong><span>${r.tipo}</span></p><p><strong>Escritura/Termo (ID):</strong><span>${r.idEscritura || 'Não informado'}</span></p></div></div>`).join('')}</div>`;
        }
        ...
        if (hasBens) {
            ...
            htmlSections += `<div class="preview-section"><h3>${sectionCounter}. Relação de Bens, Direitos e Dívidas</h3>
                ...
                ${data.bens.valoresResiduais.length > 0 ? `<h4>Valores Residuais</h4>${data.bens.valoresResiduais.map(item => `<div class="preview-card-small"><p><strong>Tipo:</strong><span>${item.tipo}</span></p><p><strong>Instituição:</strong><span>${item.instituicao || 'N/A'}</span></p><p><strong>Valor (R$):</strong><span>${item.valor || 'N/A'}</span></p><p><strong>Doc. (ID):</strong><span>${item.idDocumento || 'Pendente'}</span></p></div></div>`).join('')}`: ''}
                ${data.bens.dividas.length > 0 ? `<h4>Dívidas do Espólio</h4>${data.bens.dividas.map(item => `<div class="preview-card-small"><p><strong>Credor:</strong><span>${item.credor}</span></p><p><strong>Tipo:</strong><span>${item.tipo}</span></p><p><strong>Valor (R$):</strong><span>${item.valor}</span></p><p><strong>Doc. (ID):</strong><span>${item.idDocumento || 'Pendente'}</span></p></div></div>`).join('')}`: ''}
        ...
        if (data.documentacaoTributaria.length > 0 || (data.custas && data.custas.situacao)) {
            ...
            ${data.documentacaoTributaria.map(trib => `<div class="preview-card no-break"><h4>Tributos de: <strong>${trib.nomeFalecido}</strong></h4><p><strong>Status ITCD:</strong><span>${trib.statusItcd}</span></p><p><strong>CND Municipal:</strong><span>${trib.cndMunicipal.status === 'Juntada' ? `Juntada (ID: ${trib.cndMunicipal.id || 'N/A'})` : 'Não Juntada'}</span></p><p><strong>CND Estadual:</strong><span>${trib.cndEstadual.status === 'Juntada' ? `Juntada (ID: ${trib.cndEstadual.id || 'N/A'})` : 'Não Juntada'}</span></p><p><strong>CND Federal:</strong><span>${trib.cndFederal.status === 'Juntada' ? `Juntada (ID: ${trib.cndFederal.id || 'N/A'})` : 'Não Juntada'}</span></p></div></div>`).join('')}
            ${data.custas ? `<div class="preview-card"><h4>Custas Processuais</h4><p><strong>Situação:</strong><span>${getCustasStatus(data.custas)}</span></p></div></div>` : ''}
            </div>`;
        }
        if (data.observacoes.length > 0) {
          sectionCounter++;
          htmlSections += `<div class="preview-section"><h3>${sectionCounter}. Observações Adicionais</h3>${data.observacoes.map(obs => `<div class="preview-card no-break obs-${obs.relevancia.toLowerCase()}"><p><strong>${obs.titulo || 'Observação'}</strong></p><p class="obs-content"><span>${obs.conteudo}</span></p></div></div>`).join('')}</div>`;
        }
...
</style></head><body><div id="pdf-container"><div class="preview-header"><div class="header-text"><p>PODER JUDICIÁRIO DO ESTADO DE MINAS GERAIS</p><p class="comarca">VARA ÚNICA DA COMARCA DE NOVA RESENDE/MG</p></div></div><h1>CERTIDÃO DE REGULARIDADE</h1><h2 class="subtitle">INVENTÁRIO</h2></div><div class="preview-content">${pendencies && pendencies.length > 0 ? `<div class="preview-section pendencies-section"><h3>PENDÊNCIAS</h3><ul class="pendencies-list">${pendencies.map(p => `<li>${p}</li>`).join('')}</ul></div>` : ''}${htmlSections}</div><div class="preview-footer"><div class="signature-line"><p>${data.processo.responsavel.nome || '________________________________'}</p><p>${data.processo.responsavel.cargo || 'Cargo do Responsável'}</p></div></div></div></div></body></html>`;
